...
- 修复音域回响只在副界面可见的问题；主界面各组件缝隙现在可持续看到同一回响场景，并保持粒子、频谱、歌词和 3D 歌单层级。
- 新增长按高潮预览与绝对截止时间睡眠计时：复用唯一播放器、稳定循环，并在结束或取消时恢复原队列、曲目和播放状态。
- 新增 Windows 原生任务栏播放/暂停/上一首/下一首控制，并补全搜索加载态、完整性告警确认和主进程可信边界。
- 新增可撤销、去重且原子回滚的预设分享码；账号、音乐登录和本机私有壁纸不会进入共享预设。
- Electronic Pet 2 改为固定 Bible Strong Avatar Lab AGPL 源码快照、本地运行时与确定性构建，完整对应源码和许可文本随源码发行。
- 优化开屏、Home、搜索、歌单、壁纸视频、频谱和音域回响的调度与布局读取；60 秒性能门禁和四档 DPI 窗口回归通过。
- 移除金色星轨产品预设及运行时入口，仅保留项目所有者原创来源历史记录；刷新全部第三方组件与生产依赖许可证门禁。
- Electron 源码回归、生产/完整 `npm audit`、后端/UI/安装包完整性与真实安装验证全部纳入发布流程。

## v1.1.43

- 完成 LumiField V4 全量功能收口：交互开屏、动态搜索与天气、账号切换、音域回响、歌词消散、3D 歌单、语音助手和视觉控制台统一进入正式发行树。
- 修复开屏一次点击交接与黑屏水印问题，启动渲染增加后台暂停、帧率分档和 reduced-motion 降级。
- 副界面频谱改为抗锯齿圆角柱体并校正与粒子、歌词、3D 歌单的层级及鼠标相机响应。
- 五平台账号状态采用严格 `sessionValid`、作用域隔离、可信 IPC、原子偏好存储和登录/二维码竞态防护。
- Electron 更新至 42.9.0，`@electron/rebuild` 更新至 4.2.0；生产与完整 npm audit 均为 0。
- 移除 GSAP 第三方运行时，以 LumiField 原创轻量动画兼容层替代。
- 完成依赖许可证、21st 组件、音域回响、原创资产和参考素材的逐项来源记录；当前公开发行树达到 GPLv3 发布条件。
- 构建加入可复现版本指纹、签名公钥规范化、安装包完整性清单与打包 smoke。
