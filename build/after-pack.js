...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for LumiField icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'LumiField';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`LumiField executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`LumiField icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting LumiField resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', productName,
    '--set-version-string', 'ProductName', productName,
    '--set-version-string', 'CompanyName', 'LumiField Contributors',
    '--set-version-string', 'OriginalFilename', `${appName}.exe`,
    '--set-file-version', version,
    '--set-product-version', version
  ], { stdio: 'inherit' });

  console.log('  • hardening LumiField Electron fuses');
